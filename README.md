...
**✨ Recent Updates:**
- **Fixed:** Critical rollout serialization bug (nodes now properly share training data)
- **Verified:** Rollout files contain actual text completions, not empty objects
...
## Verifying Rollout Sharing

To verify that nodes are properly sharing training data, check the rollout files:

```bash
# View a rollout file to verify it contains actual text data
cat /path/to/gdrive/experiments/{EXPERIMENT_NAME}/rollouts/round_0/stage_0/node_1.json
```

**Valid rollout files should contain:**
- `world_state`: Question/answer pairs from the dataset
- `actions`: List of text completions generated by the model
- `metadata`: Rewards and other training metrics

**Example of valid rollout data:**
```json
{
  "rollouts": {
    "12345": [{
      ...
      "actions": ["The answer is 4", "Two plus two equals four"],
      "metadata": {"reward": 1.0}
    }]
  }
}
...
**⚠️ Invalid rollout (bug fixed in latest version):**
If you see empty objects like `{}` in the rollout files, you're running an older version. Update to the latest code from the main branch.
...
**Rollout files contain empty `{}` objects:**
- This bug was fixed in the latest version (commit 47f46b0)
- Update to the latest code: `git pull origin main`
- Delete old experiment data and re-run: `rm -rf /path/to/gdrive/experiments/{OLD_EXPERIMENT}`
- Verify fix: Check rollout files contain `"actions"` with actual text completions
